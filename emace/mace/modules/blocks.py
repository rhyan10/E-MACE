...
class LinearDipoleReadoutBlock(torch.nn.Module):
    def __init__(self, irreps_in: o3.Irreps, n_energies: int, dipole_only: bool = False):
        super().__init__()
        if dipole_only:
            self.irreps_out = o3.Irreps(str(n_energies**2 - n_energies)+"x1o")
        else:
            self.irreps_out = o3.Irreps(str(n_energies)+"x0e + "+str(n_energies**2 - n_energies)+"x1o")
...
@compile_mode("script")
class NacsLinearReadoutBlock(torch.nn.Module):
    def __init__(self, irreps_in: o3.Irreps, n_energies: int, nacs_energy_only: bool = False):
        super().__init__()
        self.irreps_out = o3.Irreps(str(int(n_energies*(n_energies-1)/2))+"x0e")
        self.linear = o3.Linear(irreps_in=irreps_in, irreps_out=self.irreps_out)
...
@compile_mode("script")
class NacsNonLinearReadoutBlock(torch.nn.Module):
    def __init__(
        self, irreps_in: o3.Irreps, MLP_irreps: o3.Irreps,  n_energies: int, gate: Optional[Callable]
    ):
        super().__init__()
        self.hidden_irreps = MLP_irreps
        self.linear_1 = o3.Linear(irreps_in=irreps_in, irreps_out=self.hidden_irreps)
        self.non_linearity = nn.Activation(irreps_in=self.hidden_irreps, acts=[gate])
        self.linear_2 = o3.Linear(
            irreps_in=self.hidden_irreps, irreps_out=o3.Irreps(str(int(n_energies*(n_energies-1)/2))+"x0e")
        )
