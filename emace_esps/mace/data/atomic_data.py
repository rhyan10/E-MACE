...
class AtomicData(torch_geometric.data.Data):
    ...
    def __init__(
        ...
        coeff_weight: Optional[torch.Tensor],
    ...
    def from_config(
        ...
        coeff_weight = (
            torch.tensor(config.coeff_weight, dtype=torch.get_default_dtype())
            if config.coeff_weight is not None
            else 1
        )
        ...
        coeff = (
            torch.tensor(config.coeff, dtype=torch.get_default_dtype())
