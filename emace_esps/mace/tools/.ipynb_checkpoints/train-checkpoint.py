    ...
    while epoch < max_num_epochs:
        ...
        if epoch % eval_interval == 0:
            ...
            if log_errors == "PerAtomRMSE":
                ...
                error_f = eval_metrics["mae_f"] * 1e3
                error_coeff = eval_metrics["mae_coeff"]* 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, MAE_E={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A, MAE_coeff ={error_coeff:.3f}"
                )
                ...
                logging.info(
                     f"Epoch {epoch}: loss={valid_loss:.4f}, MAE_E={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A, MAE_mu={error_mu:.2f} mDebye"
                )
            ...
            elif log_errors == "EnergyNacsDipoleRMSE":
                error_e = eval_metrics["rmse_e_per_atom"] * 1e3
                error_f = eval_metrics["rmse_f"] * 1e3
                error_mu = eval_metrics["rmse_mu_per_atom"] * 1e3
                error_nacs = eval_metrics["rmse_nacs"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A, RMSE_Mu_per_atom={error_mu:.2f} mDebye, RMSE_Nacs={error_nacs:.1f}"
                )
                ...
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, MAE_E={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A, MAE_mu={error_mu:.2f} mDebye, MAE_Nac={error_nacs:.1f}"
                )
            if log_wandb:
                wandb_log_dict = {
                    "epoch": epoch,
...
def take_step(
    ...
    loss = loss_fn(pred=output, ref=batch, training=True)
    #print(loss)
    loss.backward()
    if max_grad_norm is not None:
...
def evaluate(
    ...
    for batch in data_loader:
        ...
        if output.get("energy") is not None and batch.energy is not None:
            ...
            delta_coeff_list.append(batch.coeff - output["coeff"])
            delta_es_per_atom_list.append(
                (batch.coeff - output["coeff"]) / (batch.ptr[1:] - batch.ptr[:-1]).unsqueeze(-1)
            )
        ...
        if output.get("nacs") is not None and batch.nacs is not None:
            nacs_computed = True
            delta_nacs_list.append(batch.nacs - output["nacs"])
            nacs_list.append(batch.nacs)
    ...
    if E_computed:
        ...
        delta_coeff = to_numpy(torch.cat(delta_coeff_list, dim=0))
        ...
        aux["mae_coeff"] = compute_mae(delta_coeff)
    if Fs_computed:
    ...
    if nacs_computed:
        delta_nacs = to_numpy(torch.cat(delta_nacs_list, dim=0))
        nacs = to_numpy(torch.cat(nacs_list, dim=0))
