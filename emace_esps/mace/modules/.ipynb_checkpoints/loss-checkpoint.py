...
def mean_squared_error_energy(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # energy: [n_graphs, ]
    return torch.mean(torch.sum(torch.square(ref["energy"] - pred["energy"]), dim = -1))  # []

def weighted_mean_squared_error_coeff(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # energy: [n_graphs, ]
    configs_weight = ref.weight  # [n_graphs, ]
    configs_energy_weight = ref.coeff_weight  # [n_graphs, ]
    ...
    return torch.mean(
        ...
        * torch.sum(torch.square((ref["coeff"] - pred["coeff"])) / num_atoms.unsqueeze(-1), dim=-1)
    )  # []
...
def mean_squared_error_forces(ref: Batch, pred: TensorDict) -> torch.Tensor:
    ...
    return torch.mean(
        configs_weight.unsqueeze(-1)
        * configs_forces_weight.unsqueeze(-1)
        * torch.sum(torch.square(ref["forces"] - pred["forces"]), dim=-1)
    )  # []

def mean_squared_error_nacs(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # nacs: [n_energies*(n_energies-1)/2), 3]
    configs_weight = torch.repeat_interleave(
        ref.weight, ref.ptr[1:] - ref.ptr[:-1]
    ).unsqueeze(
    ...
    configs_nacs_weight = torch.repeat_interleave(
        ...
        -1
    )  # [n_energies*(n_energies-1)/2), 1]
    return torch.mean(
        configs_weight.unsqueeze(-1)
        * configs_nacs_weight.unsqueeze(-1)
        * torch.square(ref["nacs"] - pred["nacs"])
    )  # []
...
def weighted_mean_squared_error_dipole(ref: Batch, pred: TensorDict) -> torch.Tensor:
    ...
    return torch.mean(torch.square((ref["dipole"] - pred["dipole"]) / num_atoms.unsqueeze(-1)))  # []
...
class WeightedEnergyForcesLoss(torch.nn.Module):
    def __init__(self, coeff_weight = 1.0 ,energy_weight=0.0, forces_weight=1.0) -> None:
        super().__init__()
        ...
        self.register_buffer(
            "coeff_weight",
            torch.tensor(coeff_weight, dtype=torch.get_default_dtype()),
        )

    def forward(self, ref: Batch, pred: TensorDict, training=bool) -> torch.Tensor:
        #if training == True:
        return self.forces_weight * mean_squared_error_forces(ref, pred) + self.coeff_weight * weighted_mean_squared_error_coeff(ref, pred) * 0.99 + self.energy_weight * weighted_mean_squared_error_energy(ref, pred) * 0.01
        
        # else:
        #     return self.energy_weight * weighted_mean_squared_error_energy(ref, pred) + self.forces_weight * mean_squared_error_forces(ref, pred)
...
class WeightedEnergyForcesNacsDipoleLoss(torch.nn.Module):
    def __init__(self, energy_weight=1.0, forces_weight=1.0, dipole_weight=1.0, nacs_weight=1.0) -> None:
        super().__init__()
        self.register_buffer(
            "energy_weight",
            torch.tensor(energy_weight, dtype=torch.get_default_dtype()),
    ...
    def forward(self, ref: Batch, pred: TensorDict) -> torch.Tensor:
        return (
            self.energy_weight * weighted_mean_squared_error_energy(ref, pred)
            + self.forces_weight * mean_squared_error_forces(ref, pred)
            + self.nacs_weight * phase_rmse_loss(ref, pred) * 10
            + self.dipole_weight * weighted_mean_squared_error_dipole(ref, pred)
        )
