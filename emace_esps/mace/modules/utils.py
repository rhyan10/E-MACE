...
def compute_forces(
    energy: torch.Tensor, positions: torch.Tensor, training: bool = True, compute_nac: bool = False,
) -> torch.Tensor:
    split_energy = torch.split(energy, 1, dim=1)
    grad_outputs: List[Optional[torch.Tensor]] = [torch.ones_like(split_energy[0])]
    energy_grads = []
    retain_cond = True

    for i in range(len(split_energy)):
        if i == len(split_energy) - 1 and compute_nac == False:
            retain_cond = training

        gradient = torch.autograd.grad(
            outputs=[split_energy[i]],  # [n_graphs, ]
            inputs=[positions],  # [n_nodes, 3]
            grad_outputs=grad_outputs,
            retain_graph=retain_cond,  # Make sure the graph is not destroyed during training
        ...
        #gradient = torch.unsqueeze(gradient, 1)
        energy_grads.append(gradient)
    
    energy_grads = torch.stack(energy_grads, dim=1)   
    return -1 * energy_grads
...
def get_nacs(nacs: torch.Tensor, positions: torch.Tensor, training: bool = True):
    
    split_nacs = torch.split(nacs, 1, dim=1)
    grad_outputs: List[Optional[torch.Tensor]] = [torch.ones_like(split_nacs[0])]
    nacs_grads = []
    retain_cond = True
    for i in range(len(split_nacs)):
        if i == len(split_nacs) - 1:
            retain_cond = training
        
        gradient = torch.autograd.grad(
            outputs=[split_nacs[i]],
            inputs=[positions],
            grad_outputs=grad_outputs,
            retain_graph=retain_cond,  # Make sure the graph is not destroyed during training
            create_graph=training,  # Create graph for second derivative
        ...
        if gradient is None:
            return torch.zeros_like(positions)
        #gradient = torch.unsqueeze(gradient, 1)
        nacs_grads.append(gradient)
    
    nacs_grads = torch.stack(nacs_grads, dim=1)   
    return nacs_grads
...
def compute_mean_rms_energy_forces(
    ...
    for batch in data_loader:
        ...
        atom_energy_list.append(
            (batch.energy - graph_e0s.unsqueeze(-1)) / graph_sizes.unsqueeze(-1)
        )  # {[n_graphs], }
