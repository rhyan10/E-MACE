...
def main() -> None:
    ...
    if args.model == "AtomicDipolesMACE":
        ...
        if args.model == "EnergyDipolesMACE":
            ...
            args.compute_stress = False
            compute_nacs = False
        elif args.model == "EnergyNacsDipolesMACE":
            compute_dipole = True
            compute_energy = True
            compute_nacs= True
            args.compute_forces = True
    ...
    if args.loss == "weighted":
        loss_fn = modules.WeightedEnergyForcesLoss(
            coeff_weight = args.coeff_weight, energy_weight=args.energy_weight, forces_weight=args.forces_weight
        )
    elif args.loss == "excited_weighted":
        loss_fn = modules.ExcitedWeightedEnergyForcesLoss(
    ...
    elif args.loss == "energy_forces_nacs_dipole":
    ...
    if args.model == "MACE":
        ...
        assert (
            args.error_table == "EnergyDipoleRMSE" or "DipoleMAE"
        ...
        # std_df = modules.scaling_classes["rms_dipoles_scaling"](train_loader)
        assert (
            args.error_table == "EnergyNacsDipoleRMSE" or args.error_table == "EnergyNacsDipoleMAE"
        ), "Use error_table EnergyNacsDipoleRMSE with EnergyNacsDipolesMACE model"
        model = modules.EnergyNacsDipolesMACE(
            **model_config,
            n_energies=args.n_energies,
            correlation=args.correlation,
            gate=modules.gate_dict[args.gate],
            ...
            MLP_irreps=o3.Irreps(args.MLP_irreps),
            Nacs_MLP_irreps=o3.Irreps(args.MLP_irreps),
        )
    else:
    ...
    if args.model == "EnergyNacsDipolesMACE":
        param_options = dict(
        params=[
            {
                "name": "embedding",
            ...
            {
                "name": "nacs_readouts",
                "params": model.nacs_readouts.parameters(),
                "weight_decay": 0.0,
            },
        ],
        lr=args.lr,
    ...
    if args.swa:
        ...
        if args.loss == "forces_only":
            ...
            logging.info(
                f"Using stochastic weight averaging (after {args.start_swa} epochs) with energy weight : {args.swa_energy_weight}, forces weight : {args.swa_forces_weight}, dipole weight : {args.swa_dipole_weight}, dipole weight : {args.swa_nacs_weight} and learning rate : {args.swa_lr}"
            )
